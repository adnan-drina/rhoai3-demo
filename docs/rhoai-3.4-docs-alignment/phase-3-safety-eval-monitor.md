...
| LM-Eval | Evaluate LLMs with benchmark tasks and metrics. | LMEvalJob, TrustyAI operator, model endpoint access, online/code execution controls. | `trustyai.opendatahub.io/v1alpha1` LMEvalJob remains the documented API. | `covered`: Step 08 implements LM-Eval and validates RAG/model evaluation assets. |
| Guardrails | Protect inputs/outputs with safety controls. | TrustyAI, NemoGuardrails, guardrails config, model endpoint, route/API. | RHOAI 3.4 documents NeMo Guardrails as the aligned path; support status must be explicit. | `covered`: Step 09 migrated to `NemoGuardrails` and notes Technology Preview scope. |
...
| Step 08 Model Evaluation | Strong alignment with LM-Eval and repeatable benchmark/evaluation story. |
...
| P3 | Add optional synthetic data/RAG evaluation references from rh-brain to explain future quality loops. | Step 08 |
