...
| F-03 | MLflow should be the evaluation and MLOps evidence record, not just an installed server. | Step 12 logs training lifecycle runs; Step 08 now configures an `enterprise-rag` MLflow workspace and logs RAG evaluation metrics, params, tags, and compact JSON evidence from the KFP RAG evaluation pipeline when the MLflow server is present. | Validate the new Step 08 MLflow run on the live cluster, then consider feeding GuideLLM results and future EvalHub jobs into MLflow. |
...
|---|---|---|---|---|
| MLflow Operator managed in `DataScienceCluster`; MLflow SDK pre-installed in workbench/runtime images | Release notes describe MLflow integration as Technology Preview and the operator as a managed component. | Operator and CRs deployed; Step 12 logs training runs and Step 08 now logs RAG evaluation evidence through the MLflow SDK with namespace authentication. | Covered for training and RAG evaluation lifecycle; serving benchmark tracking remains open | Extend MLflow evidence to GuideLLM and EvalHub jobs. |
