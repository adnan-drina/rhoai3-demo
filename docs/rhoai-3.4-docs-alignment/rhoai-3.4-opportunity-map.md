...
| F-03 | MLflow was installed but not used as a 3.4 MLOps record. | Done. Step 12 adds `log_mlflow_run`, uses the MLflow SDK with Kubernetes namespace authentication, preflights the `face-recognition` experiment, logs metrics/params/tags, and uploads compact evidence artifacts through the namespace `MLflowConfig`. Live validation found `face-recognition-20260516-142254` `FINISHED` with S3 artifact URI. | No immediate action. Future improvement: feed Step 08/GuideLLM/RAG evaluation results into MLflow as a broader evaluation record. |
...
|---|---|---|---|---|
| MLflow Operator managed in `DataScienceCluster`; MLflow SDK pre-installed in workbench/runtime images | Release notes describe MLflow integration as Technology Preview and the operator as a managed component. | Operator and CRs deployed; Step 12 pipeline logs experiment runs and compact artifacts through the MLflow SDK; live run `face-recognition-20260516-142254` is `FINISHED`. | Covered for training lifecycle | Extend MLflow evidence to evaluation and serving benchmarks. |
...
| Item | Action | Acceptance |
|---|---|---|
| MLflow lifecycle story | Done for the training lifecycle. `validate.sh` now proves the latest Step 12 MLflow run is fresh and finished. | Step 12 shows MLflow UI/API evidence for the latest training run. |
