...
| Step 09 live NeMo adoption | Guardrails | Done. Live app resources show `NemoGuardrails/nemo-guardrails`, no `GuardrailsOrchestrator` or detector `InferenceService`s remain. The previous cleanup restored `main`; the RHOAI 3.4 refactoring branch now intentionally pins Applications to `rhoai34-refactoring`. | `steps/step-09-guardrails/validate.sh` passes with 9 passed, 0 warnings, 0 failed. |
| MLflow functional story | MLOps and RAG evaluation | Done for the training lifecycle and implemented for RAG evaluation. Step 12 logs training runs through `log_mlflow_run`; Step 08 configures an `enterprise-rag` MLflow workspace and logs RAG evaluation metrics, params, tags, and compact JSON evidence through `log_rag_mlflow`. | Step 12 validation proves a fresh finished training MLflow run; Step 08 validation proves workspace wiring and checks for a fresh finished `enterprise-rag` RAG evaluation run. |
| EvalHub product-native evaluation | Evaluation | Done for the first path. Step 08 deploys EvalHub, PostgreSQL, tenant RBAC, built-in providers, route/health checks, a smoke job against `granite-8b-agent`, and MLflow experiment tracking. | Step 08 validation proves EvalHub readiness, provider discovery, a completed smoke job, and `results.mlflow_experiment_url`. |
...
| EvalHub provider expansion | Evaluation | Add Garak/RAGAS/custom provider demos, OCI export, EvalHub MCP, and EvalHub observability after the initial smoke path is stable. | Step 08 moves beyond smoke into safety, RAG, and custom-provider evaluation with MLflow and optional OCI evidence. |
