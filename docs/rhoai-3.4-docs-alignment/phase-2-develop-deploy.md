...
| Models-as-a-Service | Govern LLM access and model deployment lifecycle. | MaaS, Gateway, AuthPolicy, model catalog, vLLM serving. | The current RHOAI 3.4 MaaS product guide labels MaaS as Technology Preview; vLLM-on-MaaS, external OIDC, external models, and observability stay preview-scoped. | `covered`: Steps 02 and 05 implement MaaS and LLM serving with explicit preview caveats. |
