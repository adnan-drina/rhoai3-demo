...
| Models-as-a-Service | Govern LLM access and model deployment lifecycle. | MaaS, Gateway, AuthPolicy, model catalog, vLLM serving. | Core MaaS is GA in the RHOAI 3.4 release notes; vLLM-on-MaaS, external OIDC, external models, and observability remain preview-scoped subfeatures. | `covered`: Steps 02 and 05 implement MaaS and LLM serving with explicit subfeature caveats. |
