...
- **TrustyAI SPD "No available data" in Dashboard** -- The SPD metric is configured and visible in the RHOAI Dashboard Model bias tab, but shows "No available data." Root cause: TrustyAI distinguishes "uploaded data" (training/reference) from "intercepted predictions" (live inference). Only intercepted predictions populate the SPD chart. Interception requires the KServe payload logger, which in RHOAI 3.3 with RawDeployment mode does not successfully forward payloads to TrustyAI for OpenVINO models. The [official TrustyAI demos](https://github.com/trustyai-explainability/odh-trustyai-demos/tree/main/3-DataDrift) use KServe Serverless mode with Istio sidecar injection, which is deprecated in RHOAI 3.3. The [rhoai-mlops deploy-lab rhoai33-updates branch](https://github.com/rhoai-mlops/deploy-lab/tree/rhoai33-updates) deploys a `kserve-logger-ca-bundle` ConfigMap but their monitoring works with tabular MLServer models, not OpenVINO/ONNX vision models with large tensor payloads.
- **MLServer proxy was evaluated and removed** -- We deployed a scikit-learn metrics proxy via MLServer. Dashboard showed "Metrics not supported" for custom runtimes on Endpoint performance tab. Removed in favor of direct SPD configuration on the YOLO model.
- **What works today:** Endpoint performance (request count, latency), SPD metric configured in Dashboard (visible but no data), pipeline-based quality tracking (mAP50 across 3 versions in Model Registry).
