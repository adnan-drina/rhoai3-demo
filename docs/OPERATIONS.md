...
- `stage-230-private-data-rag/run-autorag-pipeline.sh` imports the vendored
  ...
  `rhoai-3.4`, executor images aligned to the installed operator CSV
  `odh_autorag_image` digest) with the documented AutoRAG pipeline name,
  submits an optimization run against the scoped
  `autorag/rhoai-product-docs/input/` corpus and committed
  validate-and-protect benchmark, reviews leaderboard and RAG pattern
...
- upload repo-stored source PDFs to the project bucket under
  `raw/rhoai-product-docs/`, the AutoRAG benchmark JSON under
  `autorag/rhoai-product-docs/`, and the scoped AutoRAG input PDFs under
  `autorag/rhoai-product-docs/input/` using an in-cluster Job that clones the
  same Git branch as Argo CD
...
against the MaaS-governed Nemotron model, registers it as a Llama Stack shield
for the Stage 230 chatbot, and exports OpenTelemetry traces to a stage-local
Tempo instance.

### Environment prerequisites

Guarded generation and self-check rails need Nemotron running. If the
environment is parked, un-park it first (cluster-admin for the MachineSet;
see `env-manage-resources`):

```bash
oc scale machineset <gpu-machineset> -n openshift-machine-api --replicas=1
# wait for the GPU node to be Ready and NVIDIA operator pods to start
oc patch llminferenceservice nemotron-3-nano-30b-a3b -n models-as-a-service \
  --type=merge -p '{"spec":{"replicas":1}}'
```

`deploy.sh` checks Nemotron readiness and fails with these instructions; it
never scales GPU infrastructure itself. Re-park with the reverse operations
when the demo is idle.

### MaaS API key rotation

The guardrails model token is minted by `deploy.sh` against the
`ai-safety-guardrails` subscription and stored only in the
`nemo-guardrails-api-token` Secret. To rotate it:

```bash
oc delete secret nemo-guardrails-api-token -n ai-safety
./stage-240-guardrails-and-safety/deploy.sh
```

The script mints a fresh `sk-oai-*` key (as `ai-developer`) and restarts the
NeMo deployment. Changing `RHOAI_STAGE240_MAAS_SUBSCRIPTION` also forces a
fresh key so the new rate limits apply.

### Authenticated guardrails calls

All service endpoints require a bearer token when
`security.opendatahub.io/enable-auth` is on:

```bash
GUARDRAILS_ROUTE=https://$(oc get route nemo-guardrails -n ai-safety -o jsonpath='{.status.ingress[0].host}')
...
`status: blocked` with `rails_status` names the rail that fired. Use
`/v1/chat/completions` with the same headers for guarded generation.

### Traces

The Jaeger UI route in `ai-safety` (created by the `TempoMonolithic`
`jaegerui.route`) shows `nemo-guardrails` spans: rail execution order, LLM
call latency, and block decisions. Trace content capture is enabled for demo
visibility (`enable_content_capture: true` in the NeMo config); disable it
outside demos.

The `maas-internal-proxy-config` ConfigMap carries the environment-specific
gateway hostname and is regenerated by every `deploy.sh` run, exactly like
the Stage 230 proxy.
