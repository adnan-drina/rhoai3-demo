...
Step-09 proved your RAG system can retrieve and answer. But how do you know the answers stay grounded after you change chunk sizes, swap embedding models, or ingest new documents? Step-10 adds a repeatable evaluation pipeline that scores faithfulness, relevancy, and tool behavior -- then publishes human-readable reports.
...
No new infrastructure is deployed. This step reuses `lsd-rag` (step-09) for both
RAG execution and scoring, and `dspa-rag` (step-09) for pipeline orchestration.

## Prerequisites

```bash
# Step-09 RAG infrastructure must be deployed and healthy
...
**Solution:** Run the step-09 ingestion pipeline first to populate Milvus collections.
