...
# Deploys enterprise AI project infrastructure:
# - MinIO S3 storage provider
# - Authentication (HTPasswd, OAuth, Groups)
# - RHOAI Data Connections for MinIO
# - Project RBAC (ai-admin, ai-developer)
# - Kueue LocalQueue for the maas namespace
set -euo pipefail
...
STEP_NAME="step-03-enterprise-projects"

load_env
check_oc_logged_in

log_step "Step 03: Enterprise Projects and Storage Foundation"
...
log_step "Creating Argo CD Application for enterprise projects"
...
oc apply -f "$REPO_ROOT/gitops/step-03-enterprise-projects/base/minio/route.yaml"
...
for ns in maas enterprise-rag enterprise-mlops; do
    until oc get namespace "$ns" &>/dev/null; do
        log_info "Waiting for $ns namespace..."
        sleep 5
    done
    log_success "Namespace '$ns' created"

    if [[ "$ns" == "maas" ]]; then
        oc label namespace maas kueue.openshift.io/managed=true --overwrite &>/dev/null
        log_success "Namespace 'maas' labelled for Kueue management"
    fi

    until oc get secret minio-connection -n "$ns" &>/dev/null; do
        log_info "Waiting for MinIO Data Connection in $ns..."
        sleep 5
    done
    log_success "Data Connection 'minio-connection' created in $ns"
done

if oc get localqueue maas-default -n maas &>/dev/null; then
    log_success "Kueue LocalQueue 'maas-default' created in maas"
else
    log_warn "Kueue LocalQueue not visible yet; verify the Red Hat build of Kueue Operator is ready"
fi
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "Enterprise AI Project Foundation Deployed"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "Demo Users:"
echo "  ┌──────────────┬─────────────┬────────────────────┐"
...
echo ""
echo "Projects:"
echo "  • maas              Shared LLM serving and MaaS endpoints"
echo "  • enterprise-rag    RAG, Llama Stack, MCP, guardrails"
echo "  • enterprise-mlops  Predictive AI, pipelines, MLflow, TrustyAI"
echo ""
echo "Kueue:"
echo "  • ClusterQueue: maas-gpu"
echo "  • LocalQueue:   maas/maas-default"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
echo "  for ns in maas enterprise-rag enterprise-mlops; do oc get secret -n \$ns -l opendatahub.io/connection-type=s3; done"
echo ""
echo "  # Verify Kueue queue for MaaS"
echo "  oc get localqueue maas-default -n maas"
echo ""
log_info "Test login:"
