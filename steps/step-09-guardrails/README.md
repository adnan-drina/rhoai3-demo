...
oc apply -f gitops/argocd/app-of-apps/step-09-guardrails.yaml
...
- `gitops/step-05-*/llamastack.yaml` -- `FMS_ORCHESTRATOR_URL` + shields
- `gitops/step-07-*/llamastack-rag.yaml` -- `FMS_ORCHESTRATOR_URL` + shields

## Rollback / Cleanup

```bash
# Delete ArgoCD Application (cascading delete)
oc delete application step-09-guardrails -n openshift-gitops
...
> **Design Decision:** Three detectors (regex + HAP + prompt injection) instead of four. The language detector from the rhoai-genaiops workshop adds a fourth InferenceService with minimal demo value. The RHOAI 3.3 docs examples use this trio.
...
> **Design Decision:** Both LlamaStack distributions updated with `FMS_ORCHESTRATOR_URL` and shield registrations. This enables guardrails in the Playground UI (step-05) and in RAG agent API calls (step-07).

> **Note (RHOAI 3.3):** When step-09 is NOT deployed, both LSDs will log connection warnings for `FMS_ORCHESTRATOR_URL` but inference and RAG work normally. Shields activate only when the Orchestrator is running.
...
- [RHOAI 3.3 -- Enabling AI Safety with Guardrails](https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.3/html/enabling_ai_safety_with_guardrails/enabling-ai-safety-with-guardrails_safety)
