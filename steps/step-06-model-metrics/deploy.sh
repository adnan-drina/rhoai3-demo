...
if oc get dspa dspa-rag -n "$NAMESPACE" &>/dev/null; then
    ...
    if [ -f "$REPO_ROOT/artifacts/bench-qwen3-8b.yaml" ] && [ -f "$REPO_ROOT/artifacts/bench-mistral-bf16.yaml" ]; then
        log_success "Pipelines compiled: bench-qwen3-8b.yaml, bench-mistral-bf16.yaml"
...
echo "    Develop & train → Pipelines → bench-qwen3-8b → Create run"
