...
check "ModelRegistry CR exists" \
    "oc get modelregistry.modelregistry.opendatahub.io enterprise-ai-registry -n rhoai-model-registries -o jsonpath='{.metadata.name}'" \
    "enterprise-ai-registry"

MR_PODS=$(oc get pods -n rhoai-model-registries -l app=enterprise-ai-registry --no-headers 2>/dev/null | grep -c "Running" || echo "0")
...
SVC_EXISTS=$(oc get svc enterprise-ai-registry-internal -n rhoai-model-registries -o jsonpath='{.metadata.name}' 2>/dev/null || echo "")
if [[ "$SVC_EXISTS" == "enterprise-ai-registry-internal" ]]; then
    echo -e "${GREEN}[PASS]${NC} Internal service exists"
    VALIDATE_PASS=$((VALIDATE_PASS + 1))
else
    echo -e "${RED}[FAIL]${NC} Internal service enterprise-ai-registry-internal not found"
    VALIDATE_FAIL=$((VALIDATE_FAIL + 1))
fi
