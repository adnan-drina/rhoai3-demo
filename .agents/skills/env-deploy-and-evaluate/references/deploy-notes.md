...
16. **KServe webhook availability during registry outages** — If `oc patch` or pod admission fails with `no endpoints available for service "kserve-webhook-server-service"`, check `kserve-controller-manager` in `redhat-ods-applications`. During registry.redhat.io 502/504 incidents, a temporary live-only incident response might restart operator-managed pods on nodes that already have cached images. Do not commit controller Deployment image or `imagePullPolicy` patches as desired state. Make the durable fix through operator lifecycle, registry health, or product-managed state once the registry is healthy.
