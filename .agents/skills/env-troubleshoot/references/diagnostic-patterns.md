...
| Stage 230 Application remains OutOfSync on Kueue labels | Kueue injected controller-owned labels/annotations, or a live controller already admitted to the immutable `default` queue is being compared with a custom `lq-*` queue label | `oc get deployment private-rag-docling -n demo-sandbox -o yaml`; keep Stage 230 runtime controllers on `default` and ignore only Kueue bookkeeping fields |
