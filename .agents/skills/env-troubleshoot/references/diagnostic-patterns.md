...
| granite-8b-agent CrashLoop: "Repo id must be in the form 'repo_name'" | anyuid SCC on default SA breaks modelcar FUSE mount | Remove anyuid from default SA; use dedicated SA for postgres only |
...
| Mistral ToolCall index error | vLLM bug with Mistral tool_call index | Use Granite for RAG; Mistral RAG broken until vLLM fix |
...
| Chat completions model not found | Model ID must be provider-prefixed | Use `vllm-granite-agent/granite-8b-agent` |
