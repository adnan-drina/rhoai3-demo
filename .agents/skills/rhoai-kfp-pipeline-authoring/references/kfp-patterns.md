...
- use the active Llama Stack client/API shape from `rhoai-llama-stack`; do not
  assume the older `vector_stores.files.create()` path if the current stage
  uses `vector_dbs.register()` and `tool_runtime.rag_tool.insert()`
