...
| Chatbot source attribution | RAG answers render a per-guide "Sources" panel (topic tags, relevance, docs.redhat.com links) built from chunk metadata, in both Direct and Agent modes; the model is prompted to name source guides | Enterprise-grade answers must be attributable to the official documents that grounded them; the corpus metadata (guide_slug, topic, source_url, product, version) already travels with every chunk, so attribution is deterministic rather than model-claimed. |
