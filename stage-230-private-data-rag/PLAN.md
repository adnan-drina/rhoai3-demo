...
- New components:
  ...
  - Llama Stack `userConfig` adapted from the Red Hat article-linked reference
    implementation to register the generation, embedding, vector, and reranker
    providers
...
| Embeddings | Use `sentence-transformers/ibm-granite/granite-embedding-125m-english` through the inline sentence-transformers provider | Matches the article-linked reference path and keeps embedding behavior explicit before indexing |
...
| `gitops/stage-230-private-data-rag/llamastack/` | `LlamaStackDistribution` using official `rh-dev` distribution plus curated `userConfig` ConfigMap for AG News provider registration | RHOAI Llama Stack docs, installed CRD schema, and reviewed article-linked config pattern | Server-side dry run, `LlamaStackDistribution` Ready, and `/v1/models` lists Nemotron, Granite embedding, and Qwen3 reranker |
